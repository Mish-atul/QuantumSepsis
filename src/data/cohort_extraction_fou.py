"""
FOU Cohort Extraction from MIMIC-IV
====================================
...
- No obvious infection source in first 48 hours
...
class FouCohortExtractor:
    """Extract FOU cohort from MIMIC-IV."""
    ...
    def __init__(self, mimic_dir: str, output_dir: str, 
        ...
        # FOU criteria (RELAXED for better cohort size)
        self.fever_threshold = min_fever_temp  # °C (default: 38.0, was 38.3)
        self.min_fever_episodes = min_fever_count  # (default: 2, was 3)
        self.min_icu_stay_days = min_icu_days  # (default: 3, was 7)
        self.include_no_fou = include_no_fou  # Include "No FOU" class
        ...
        logger.info(f"FOU Criteria: min_icu_days={min_icu_days}, min_fever_temp={min_fever_temp}°C, min_fever_count={min_fever_count}, include_no_fou={include_no_fou}")
    ...
    def extract_cohort(self) -> pd.DataFrame:
        """Extract FOU cohort from MIMIC-IV."""
        logger.info("Starting FOU cohort extraction...")
        ...
        # Step 2: Filter by ICU stay duration ≥ 7 days
        ...
        fever_stays = self._identify_fever_episodes(temps, long_stays)
        logger.info(f"Stays with ≥{self.min_fever_episodes} fever episodes: {len(fever_stays)}")

        # Step 5: Check for negative initial cultures
        ...
        # Step 6: Exclude obvious infections
        ...
        # Step 7: Label FOU categories
        logger.info("Labeling FOU categories...")
        cohort = self._label_fou_categories(fou_candidates)
        logger.info(f"Final FOU cohort: {len(cohort)}")
        logger.info(f"Label distribution:\n{cohort['fou_label'].value_counts()}")
    ...
    def _extract_temperatures(self, stay_ids: np.ndarray) -> pd.DataFrame:
        ...
        # Load chartevents in chunks to avoid memory issues
        temps_list = []
        ...
        for chunk in pd.read_csv(
            ...
            # Filter for temperature items and relevant stays
            chunk_temps = chunk[
        ...
        # Convert Fahrenheit to Celsius if needed (values > 50 are likely Fahrenheit)
    ...
    def _identify_fever_episodes(self, temps: pd.DataFrame, icustays: pd.DataFrame) -> pd.DataFrame:
        """Identify stays with ≥3 fever episodes (>38.3°C)."""
        if temps.empty:
            return pd.DataFrame()
        ...
        # Identify fever episodes (temp > 38.3°C)
        fever_temps = temps[temps['valuenum'] > self.fever_threshold]

        # Count fever episodes per stay (group by 6-hour windows to avoid counting continuous fever as multiple episodes)
        ...
        # Filter stays with ≥3 fever episodes
    ...
    def _exclude_obvious_infections(self, stays: pd.DataFrame) -> pd.DataFrame:
        """Exclude stays with obvious infection diagnoses in first 48 hours."""
        ...
        # Obvious infection ICD codes (pneumonia, UTI, surgical site infection)
    ...
    def _label_fou_categories(self, stays: pd.DataFrame) -> pd.DataFrame:
        """Label FOU categories based on final diagnoses."""
        ...
        # Take the most specific label per stay (infectious > non-infectious > undiagnosed)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Extract FOU cohort from MIMIC-IV")
    ...
    print(f"\n=== FOU Cohort Summary ===")
    print(f"Total FOU cases: {len(cohort)}")
    print(f"\nLabel distribution:")
    print(cohort['fou_label'].value_counts())
    print(f"\nGender distribution:")
